...
export function NovoProdutoForm({
  ...
  const [grupo, setGrupo] = useState("PRO");
  ...
  function sugerirSku() {
    ...
    startTransition(async () => {
      const r = await sugerirSkuAction(grupo, nome);
      if ("erro" in r) {
      ...
      <div>
        ...
        <div className="mt-1 flex gap-2">
          <input
            ...
            className="min-w-0 flex-1 rounded-md border border-cinza-claro px-3 py-2 font-mono text-sm focus:border-ambar focus:outline-none"
          />
          <button
            type="button"
            onClick={sugerirSku}
          ...
          >
            {sugerindo ? "Pensando..." : "Sugerir SKU"}
          </button>
        </div>
